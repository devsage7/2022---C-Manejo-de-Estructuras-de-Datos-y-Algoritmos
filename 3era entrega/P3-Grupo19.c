    ...
    *ant = NULL;
    ...
    if (p == NULL)
        *exito = 0; // No Localizado.
        ...
        sumaCostosABBevoc++;  // Si el while sale por la comparación por x, sumo un costo.
        *exito = 1; // localizado.
    }
    return p;
}
...
int localizarRAL(int x, int *pos, costos *c_max, costos *c_med) {// -1 -> Celda virgen , -2 -> Celda libre.
    ...
    if (contador < M_RAL){  // Si NO recorrió M baldes...
        sumaCostosRALevoc++;    // Consultó y fracasó.
...
int localizarRS(int x, int *indiceAcc, nodo **pos, costos *c_max, costos *c_med){
    ...
    sumaCostosRSevoc++; // Costo de consultar la celda que me da el acceso a la lista.
    ...
    *pos = ant;    // Devuelvo el nodo anterior.
    if (p != NULL){
        sumaCostosRSevoc++; // Sumo el costo que no se sumó al salir por la segunda condición del while.
        return 1;   // Localizado.
    }
    else
        return 0;   // No localizado.
...
int lecturaOperaciones(costos *c_max, costos *c_medio) {
    ...
    while (!(feof(archivo))) {
        ...
        if (codigoOperador==1 || codigoOperador==2){
            ...
            auxiliar2 = auxiliar;   // Guardo la nupla para no alterarla.
            ...
            auxiliar = auxiliar2;   // Guardo la nupla para no alterarla.
...
int main() {
    ...
    /*  RESULTADOS:
        ...
        Máximos
            ...
            Evocación(Fracaso): 7.0 (En celdas consultadas)
        Medios
            Alta: 25.060 (En corrimientos de nuplas)
            Baja: 45.982 (En corrimientos de nuplas)
            Evocación: 6.596 (En celdas consultadas)
            Evocación(Fracaso): 6.8 (En celdas consultadas)

        COSTOS ABB:
        Máximos
            Alta: 3.0 (En modificaciones de punteros)
            Baja: 2.5 (En modificaciones de punteros)
            Evocación: 14.0 (En celdas consultadas)
            Evocación(Fracaso): 12.0 (En celdas consultadas)
        Medios
            Alta: 3.0 (En modificaciones de punteros)
            Baja: 1.750 (En modificaciones de punteros)
            Evocación: 8.421 (En celdas consultadas)
            Evocación(Fracaso): 3.3 (En celdas consultadas)

        COSTOS RAL:
        Máximos
            Alta: 0.0 (En corrimientos de nuplas)
            Baja: 0.0 (En corrimientos de nuplas)
            Evocación: 74.0 (En celdas consultadas)
            Evocación(Fracaso): 79.0 (En celdas consultadas)
        Medios
            Alta: 0.0 (En corrimientos de nuplas)
            Baja: 0.0 (En corrimientos de nuplas)
            Evocación: 3.013 (En celdas consultadas)
            Evocación(Fracaso): 17.1 (En celdas consultadas)

        COSTOS RS:
        Máximos
            Alta: 2.0 (En modifaciones de punteros)
            Baja: 1.0 (En modifaciones de punteros)
            Evocación: 8.0 (En celdas consultadas)
            Evocación(Fracaso): 4.0 (En celdas consultadas)
        Medios
            Alta: 2.0 (En modifaciones de punteros)
            Baja: 1.0 (En modifaciones de punteros)
            Evocación: 2.805 (En celdas consultadas)
            Evocación(Fracaso): 2.5 (En celdas consultadas)
    */
    ...
    - Corrimientos:
        ...
        necesita hacer corrimientos.

    - Cantidad de celdas consultadas:
        Evocaciones exitosas: Las mejores estructuras para evocaciones exitosas 
        en este caso son la LSOBB y el RS. Tienen un costo máximo similar (7 vs 8), pero el RS
        tiene un mejor comportamiento esperado (2.805 vs 6.596 en la LSOBB).
        Les siguen el RAL, que tiene un comportamiento esperado bueno (3.013)
        pero un costo máximo malo en comparación a las dos anteriores (74), y
        el ABB, que tiene costos decentes (14 máx y 8.421 esperado) pero es menos
        eficiente que las primeras dos.
        La LSO es la más costosa (110 max y 45.547 esperado).

        Evocaciones no exitosas: El rebalse separado es la mejor estructura sin dudas. Su costo máximo
        es de 4 y el esperado de 2.5.
        Despues del RS las mejores son el ABB (12 máx y 3.3 esperado) y la LSOBB (7 máx y 6.8 esperado). En
        este caso se podría decidir teniendo en cuenta lo que busco: Si un mejor comportamiento esperado (Mejor el
        ABB) o un peor caso menos costoso (Mejor la LSOBB).
    ...
    CONCLUSIÓN/Selección de estructura: Considerando el análisis anterior, usaría el RS porque 
    es la estructura más eficiente en evocaciones, y en las altas y bajas a pesar de no ser
    la más eficiente tiene costos muy bajos, siendo superada en esas operaciones solo por el RAL.
    */
